...
\subsection{Analisi di dati soggettivi e oggettivi}
  ...
  \begin{itemize}
    \item \textit{Task 1:} Il task è stato portato a termine senza problemi;
    \item \textit{Task 2:} Il task è stato portato a termine senza troppe difficoltà. L'utente non ha completato il task nell'immediato poichè non ha considerato di cercare nel footer. (\textcolor{red}{E1})
  ...
  \begin{itemize}
    ...
    \item \textit{Task 2:} Il task è non stato portato a termine. L'utente non ha completato il task poichè per effettuare il reso si è recata negli ordini effettuati piuttosto che nell'apposita sezione all'interno del profilo. Così è stato aggiunto il link al reso anche nella sezione degli ordini. (\textcolor{red}{E3})
    \item \textit{Task 3:} Il task è stato portato a termine con qualche difficoltà in quanto non riusciva a trovare l'apposita sezione in cui completare il task; la sezione si chiamava "Account collegati" ed essendo ambigua è stata cambiata con "Collega account young". (\textcolor{red}{E4})
  \end{itemize}
  ...
  \begin{itemize}
    \item \textit{Task 1:} Il task è stato portato a termine senza problemi;
    \item \textit{Task 2:} Il task è stato portato a termine senza troppe difficoltà. Però a task completato l'utente ha evidenziato il fatto di non ricevere alcun feedback dal sistema relativo al completamento dell'azione. (\textcolor{red}{E5})
    \item \textit{Task 3:} Il task è stato portato a termine senza problemi.
  \end{itemize}
\chapter{Conclusioni}
\chapter{Allegati}
