...
\begin{itemize}
  \item \textbf{Metodo di testing}: Discount usability test
  \item \textbf{Metodologia di testing}: Thinking Aloud
  \item \textbf{Elenco dei task da testare}: Sono stati selezionati più task da quelli offerti dal sito che messi insieme permettono di completare un'operazione. MANCANTE
  \item \textbf{Soggetti}:
  \begin{itemize}
    \item Giorgio, operaio di 24 anni, mediamente pratico con la tecnologia;
    \item Michela, parrucchiera di 42 anni, poco affine alla tecnologia;
    \item Stefano, impiegato di 56 anni, abile nell'utilizzo della tecnologia.
  \end{itemize}
  \item \textbf{Descrizione dei risultati attesi}: per effettuare una stima dei risultati otteuti sono stati applicati i criteri EEE (Efficacia, Efficienza, Emozioni); per efficacia s'intede l'accuratezza con cui gli utenti riescono a raggiungere determinati obiettivi, l'efficienza misura il tempo minimo richiesto per portare a termine un task in maniera efficace, mentre l'emozione indica il comfort e l'atteggiamento positivo dell'utente nell'utilizzo del sistema.
\end{itemize}
